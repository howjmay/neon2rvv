...
#define TEST_IMPL(IDX)                             \
  for (int i = 0; i < 1; i++) {                    \
    if (_a[i] < 0) {                               \
      ...
      if (_a[i] > ((uint64_t)UINT64_MAX >> IDX)) { \
        _c[i] = UINT64_MAX;                        \
      } else {                                     \
        _c[i] = _a[i] << IDX;                      \
      }                                            \
...
#define TEST_IMPL(IDX)                                       \
  for (int i = 0; i < 8; i++) {                              \
    uint8_t mask = UINT8_MAX >> (IDX + 1);                   \
    _c[i] = (_a[i] & ~mask) | ((_b[i] >> (IDX + 1)) & mask); \
  }                                                          \
...
#define TEST_IMPL(IDX)                                       \
  for (int i = 0; i < 4; i++) {                              \
    uint16_t mask = UINT16_MAX >> (IDX + 1);                 \
    _c[i] = (_a[i] & ~mask) | ((_b[i] >> (IDX + 1)) & mask); \
  }                                                          \
...
#define TEST_IMPL(IDX)                                       \
  for (int i = 0; i < 2; i++) {                              \
    uint32_t mask = (uint64_t)UINT32_MAX >> (IDX + 1);       \
    _c[i] = (_a[i] & ~mask) | ((_b[i] >> (IDX + 1)) & mask); \
  }                                                          \
...
#define TEST_IMPL(IDX)                                       \
  for (int i = 0; i < 1; i++) {                              \
    uint64_t mask = (uint64_t)UINT64_MAX >> (IDX + 1);       \
    if ((IDX + 1) == 64) {                                   \
      mask = 0;                                              \
    }                                                        \
    _c[i] = (_a[i] & ~mask) | ((_b[i] >> (IDX + 1)) & mask); \
...
result_t test_vtbl3_s8(const NEON2RVV_TEST_IMPL &impl, uint32_t iter) {
  ...
  int8_t _a[24];
  int8_t _c[8];
  merge_arrays(in1, in2, _in_interleave);
  // organize input array
  for (int i = 0; i < 8; i++) {
...
result_t test_vtrn_s16(const NEON2RVV_TEST_IMPL &impl, uint32_t iter) {
  ...
  return validate_int16(c, c10, c11, c12, c13, c20, c21, c22, c23);
#else
  return TEST_UNIMPL;
#endif  // ENABLE_TEST_ALL
}
...
result_t test_vtrn_u8(const NEON2RVV_TEST_IMPL &impl, uint32_t iter) {
  ...
  return validate_uint8(c, c10, c11, c12, c13, c14, c15, c16, c17, c20, c21, c22, c23, c24, c25, c26, c27);
#else
  return TEST_UNIMPL;
#endif  // ENABLE_TEST_ALL
}
...
result_t test_vtrn_u16(const NEON2RVV_TEST_IMPL &impl, uint32_t iter) {
  ...
  return validate_uint16(c, c10, c11, c12, c13, c20, c21, c22, c23);
#else
  return TEST_UNIMPL;
#endif  // ENABLE_TEST_ALL
}
...
result_t test_vst3_lane_s8(const NEON2RVV_TEST_IMPL &impl, uint32_t iter) {
  ...
  int8_t in[reg_elt_num * 4];
  merge_arrays(_in1, _in2, _in3, in);
  int8x8x3_t b;
